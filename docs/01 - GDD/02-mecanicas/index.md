# 2. Mecânicas de Jogo (Game Mechanics)

---

<details open>
<summary style="background-color: #e8e8e8; padding: 4px 8px; border-radius: 4px;"><b>⚔️ Mecânicas de Jogo</b></summary>

> O núcleo funcional do GDD. Esta seção detalha todas as mecânicas de gameplay, sistemas de personagem, combate, controles e física.

</details>

---

## Navegação

- [2.1. Core Loop (Loop Central)](core-loop.md)
- [2.2. Sistemas de Personagem](sistemas-personagem.md)
- [2.3. Sistema de Combate](sistema-combate.md)
- [2.4. Sistema de Equipamentos](equipamentos.md)
- [2.5. Sistema de Inventário](inventario.md)
- [2.6. Controles e Inputs](controles-inputs.md)
- [2.7. Física e Interação](fisica-interacao.md)
- [2.8. Sistema de Spellcasting](spellcasting.md)
- [2.9. Sistema de Rest](rest.md)
- [2.10. Sistema de Diálogos](dialogos.md)
- [2.11. Sistema de Quests](quests.md)
- [2.12. Sistema de Save/Load](save-load.md)

---

**Navegação Principal:** [← Seção Anterior: Visão e Identidade](../01-visao-identidade/index.md) | [→ Próxima Seção: IA e Comportamento](../03-ia-comportamento/index.md)
