---
title: "Design Documentation"
category: design
tags: [design, game-design, dnd-5e, rules]
last_updated: 2024-12-27
---

**Navegação:** [Home](../../README.md) > [Documentação](index.md) > Design

# Documentação de Design - MyProject2

<details open>
<summary style="background-color: #e8e8e8; padding: 4px 8px; border-radius: 4px;"><b>🎨 Visão Geral</b></summary>

> Esta seção contém toda a documentação relacionada ao **design do jogo**, incluindo:
>
> - Visão do produto final (GDD)
> - Regras D&D 5e implementadas
> - Mecânicas de gameplay
> - Design de sistemas

</details>

---

## 📚 Documentos Principais

<details>
<summary style="background-color: #e8e8e8; padding: 4px 8px; border-radius: 4px;"><b>📖 Game Design Document (GDD)</b></summary>

> **[gdd.md](gdd.md)** - Documento completo descrevendo:
>
> - Visão do jogo final (Action RPG D&D 5e)
> - Objetivos de gameplay
> - Experiência do jogador
> - Roadmap de desenvolvimento
> - Como o sistema de fichas se encaixa no jogo maior

</details>

<details>
<summary style="background-color: #e8e8e8; padding: 4px 8px; border-radius: 4px;"><b>⚔️ Regras D&D 5e</b></summary>

> **[dnd-rules/](dnd-rules/)** - Documentação algorítmica das regras do D&D 5ª edição:
>
> - Ability Scores e Modificadores
> - Sistema Point Buy
> - Raças e Sub-raças
> - Multiclassing
> - Spellcasting
> - Level Up e Features
> - Passos de Criação de Personagem
>
> **📖 Para mais detalhes, veja [dnd-rules/index.md](dnd-rules/index.md)**

</details>

<details>
<summary style="background-color: #e8e8e8; padding: 4px 8px; border-radius: 4px;"><b>🎮 Adaptação D&D 5e para Unreal Engine 5 com GAS</b></summary>

> **[gas-dnd-adaptation.md](gas-dnd-adaptation.md)** - Documento completo explicando:
>
> - Como adaptar regras de D&D 5e para Action RPG
> - Gameplay Ability System (GAS) - conceitos fundamentais
> - Comparação: RPG de Mesa vs Jogo Digital
> - Fluxos práticos (ataques, condições, magias, habilidades)
> - Sistema de colisão real e combate em tempo real
> - Adaptação de regras mantendo fidelidade
>
> **📖 Documento didático com analogias e exemplos práticos**

</details>

---

## 🎯 Navegação

<details>
<summary style="background-color: #e8e8e8; padding: 4px 8px; border-radius: 4px;"><b>🔗 Links Rápidos</b></summary>

> - **[GDD](gdd.md)** - Visão completa do jogo
> - **[Adaptação D&D 5e + GAS](gas-dnd-adaptation.md)** - Como adaptar D&D 5e para Unreal Engine 5
> - **[Regras D&D 5e](dnd-rules/)** - Referência de regras
> - **[Documentação Técnica](../technical/)** - Arquitetura e implementação
> - **[README Principal](../../README.md)** - Visão geral do projeto
> - **[ARCHITECTURE.md](../../ARCHITECTURE.md)** - Resumo da arquitetura
> - **[CHANGELOG.md](../../CHANGELOG.md)** - Histórico de mudanças
> - **[Índice de Documentação](../index.md)** - Organização completa

</details>
